...
import { pisYedeliHardAI } from './hard';
...
describe('pisYedeliHardAI', () => {
  it('prefers a play that finishes the hand over drawing', () => {
    ...
    const move = pisYedeliHardAI.chooseMove(state, 'p1', legalMoves, createRng(1));
    expect(move).toEqual({ type: 'play', cardId: 'h1' });
  });

  it('always returns a legal move across several dealt hands', () => {
    ...
    for (let seed = 1; seed <= 5; seed++) {
      ...
      const move = pisYedeliHardAI.chooseMove(state, currentPlayer, legalMoves, createRng(seed + 100));
