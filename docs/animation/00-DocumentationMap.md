...
| [`AuditTemplate.md`](./AuditTemplate.md) | Reusable template for the Architecture Audit that Workflow §2 requires before every implementation. | Before writing any animation implementation — Playground demo or production feature. |
| [`QuickAuditTemplate.md`](./QuickAuditTemplate.md) | Lighter fast-path version of the Architecture Audit, gated by its own eligibility checklist. | Only for changes that are small, single-layer, and touch already-audited behavior — anything else falls back to `AuditTemplate.md`. |
...
2. `animation-architecture-constitution.md` §1–§4 (Philosophy, Scope, Goals, Layer Model) for the "why," then §5 in full.
3. `AnimationReviewWorkflow.md` in full.
4. Whichever of `AuditTemplate.md` / `demos/DemoTemplate.md` matches the task at hand.

**Already familiar, starting a new task:**
1. `AuditTemplate.md` (or `QuickAuditTemplate.md` if it clearly qualifies) or `demos/DemoTemplate.md` (copy and fill in).
