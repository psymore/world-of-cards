...
- **Reduced-motion snap happens on every mount for a user who already has it enabled, not just on a live OS-setting toggle.** `useReducedMotion()` (`apps/mobile/src/components/useReducedMotion.ts`) always returns `false` on its very first render and only resolves the real OS value asynchronously a moment later. For a user who already has reduce-motion enabled *before* opening the app, this means the entrance effect initially runs as if reduced motion were off, the animation begins (briefly, ~1-3 frames), and then `reducedMotion` flips to `true`, the effect re-runs, and `progress.setValue(1)` hard-snaps to the rest state — a brief fade-then-snap on every single mount for these users, not only in the rarer scenario of someone toggling the OS setting live mid-animation (which also triggers the identical snap path). This is judged acceptable as-is: the partial fade before the snap is barely visible in practice and the end state (immediate settle, no continued motion) is still the correct accessibility behavior. Recorded here so this section describes the actual frequency accurately rather than understating it as an "extreme edge case."
