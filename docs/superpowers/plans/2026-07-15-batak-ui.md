...
- `GameResultModal`, `createGameSessionStore`, `useCardSelection`, `SelectableCard`, `PlayerAvatar`, and every `@world-cards/ui` export are consumed as-is — no changes to any of those files.
...
**Interfaces:**
- Produces: `@world-cards/engine/games/batak` resolvable from `apps/mobile`, exporting `batakDescriptor: GameDescriptor<BatakState, BatakMove>`, `BatakState`, `BatakMove`, `BatakSetupOptions`.
...
This file does not exist until this step, so it fails at "Cannot find module '@world-cards/engine/games/batak'" until Step 1 is done — since Step 1 already happened above, run the test now.
...
**Interfaces:**
- Consumes: `Difficulty` from `@world-cards/engine` (existing). `useSettingsStore` from `apps/mobile/src/state/settingsStore.ts` (existing, `getState().setDefaultDifficulty(value)`).
...
**Interfaces:**
- Consumes: `assignSeats`, `fanRotationDeg`, `fanCurveY`, `OPPONENT_CARD_OVERLAP`, `SIDE_CARD_STYLES`, `Seat`, `SeatPosition` from `../../table/seating` (Task 2). `PlayingCard`, `SuitIcon`, `TableFelt`, `TableWoodCorners`, `glowShadow` from `@world-cards/ui` (existing, unmodified). `SelectableCard` from `../../components/SelectableCard` (existing). `useCardSelection` from `../../components/useCardSelection` (existing). `PlayerAvatar` from `../../components/PlayerAvatar` (existing). `Card`, `Suit` from `@world-cards/engine` (existing). `BatakState`, `BatakMove` from `@world-cards/engine/games/batak` (Task 1).
...
**Interfaces:**
- Consumes: `batakDescriptor`, `BatakState`, `BatakMove` from `@world-cards/engine/games/batak` (Task 1). `createRng`, `RNG`, `Difficulty`, `PlayerId`, `Card` from `@world-cards/engine` (existing). `createGameSessionStore` from `../../state/createGameSessionStore` (existing, unmodified). `useSettingsStore` from `../../state/settingsStore` (existing). `GameScreenLayout` from `../../components/GameScreenLayout` (existing). `GameResultModal` from `../../components/GameResultModal` (existing). `useAITurn` from `../../hooks/useAITurn` (Task 2). `BatakSetupView` (Task 3). `BatakTable`, `PendingBatakPlay` (Task 4).
