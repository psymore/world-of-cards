...
- Follow existing test conventions exactly: `@testing-library/react-native`'s `render`/`screen`/`fireEvent`/`within`/`waitFor`, `await render(...)`, `clearRegistry()` from `@world-of-cards/engine` in `beforeEach` wherever registry state matters.
- Run tests with `npm test -- <path>` from the repo root (`d:\CodeSpace\world-of-cards`); it invokes the root multi-project Jest config covering both `packages/engine` and `apps/mobile`.

---

## Task 1: Expose Pişti's types and descriptor via a new engine subpath export

Today `packages/engine`'s `package.json` only whitelists two subpaths in `"exports"`: `"."` and `"./testing"`. `PistiState`/`PistiMove` and the fully-typed `pistiDescriptor` (`GameDescriptor<PistiState, PistiMove>`) are needed by the mobile UI, but nothing outside `packages/engine`'s own root barrel is currently importable. This task adds a `./games/pisti` subpath, mirroring the existing `./testing` pattern, so the UI can `import { pistiDescriptor } from '@world-of-cards/engine/games/pisti'` and get `PistiState`/`PistiMove` types alongside it.
...
**Interfaces:**
- Produces: `@world-of-cards/engine/games/pisti` resolvable from `apps/mobile`, exporting `pistiDescriptor: GameDescriptor<PistiState, PistiMove>`, `PistiState`, `PistiMove`, `PistiSetupOptions` (all re-exported from `./types`).
...
**Interfaces:**
- Consumes: `Card`, `Suit` from `@world-of-cards/engine` (already root-exported via `core/types`).
...
**Interfaces:**
- Consumes: `PlayerId`, `ScoreBoard` from `@world-of-cards/engine`.
...
**Interfaces:**
- Consumes: `GameState`, `PlayerId`, `RuleEngine`, `AIStrategy`, `RNG` from `@world-of-cards/engine`.
...
**Interfaces:**
- Consumes: `Difficulty` from `@world-of-cards/engine`; `useSettingsStore`, `defaultSettings` from `../../state/settingsStore`.
...
**Interfaces:**
- Consumes: `PistiState` from `@world-of-cards/engine/games/pisti`; `PlayingCard` from `../../components/PlayingCard`.
...
**Interfaces:**
- Consumes: `pistiDescriptor`, `PistiState`, `PistiMove` from `@world-of-cards/engine/games/pisti`; `createRng`, `Difficulty`, `PlayerId` from `@world-of-cards/engine`; `createGameSessionStore` from `../../state/createGameSessionStore`; `useSettingsStore` from `../../state/settingsStore`; `GameScreenLayout`, `GameResultModal` from `../../components/*`; `PistiSetupView`, `PistiTable`, `useAITurn` from this task's siblings (Tasks 5–7).
...
export function RootNavigator() {
  return (
    <NavigationContainer>
      <Stack.Navigator>
        <Stack.Screen name="Home" options={{ title: 'World of Cards' }}>
          {({ navigation }) => (
