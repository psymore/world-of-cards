...
| `PisYedeliTable.tsx` | Renders opponent seats, the stock pile, the discard pile (top card face-up), and the human's hand. Computes `getLegalMoves` each render to know which hand cards are tappable and whether Draw/Pass are available. |
...
1. Each render, `PisYedeliTable` calls `ruleEngine.getLegalMoves(state, humanPlayerId)` to
   get the current legal move set.
...
4. Tapping a legal Jack opens `PisYedeliSuitPickerModal`; picking a suit calls
   `performMove({ type: 'play', cardId, declaredSuit })`. Dismissing the modal without
   picking cancels — no move is made.
