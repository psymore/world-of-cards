...
- Stock-pile replenishment-when-empty is **not** in any source but is necessary for playability given "players may draw as many cards as they want" — flagged as an original addition, approved by the user alongside the rest of the design. Implemented as a plain flip (discard-minus-top becomes the new stock), not a reshuffle: the shared `RuleEngine` contract's `performMove(state, move)` receives no `RNG` (only `setup` does), and a flip needs none, since the original deal shuffle already randomized the order these cards will be redrawn in.
...
- Else if `pendingDraw > 0`: `{ type: 'play', cardId }` for each seven in hand (stacking), plus `{ type: 'draw' }` if drawing is actually possible (stock non-empty, or `discard` has more than just its top card to flip into a new stock). A player facing a penalty almost always has the option to draw it down; `{ type: 'pass' }` exists only as a fallback for the (rare, late-game) case where neither a stacking seven nor a draw is available, so the shared engine's "every in-progress state has at least one legal move" invariant always holds.
